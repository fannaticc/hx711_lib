...
void HX711_wait_ready(unsigned long time);
bool HX711_wait_ready_retry(int retries, unsigned long time);
bool HX711_wait_ready_timeout(unsigned long timeout, unsigned long time);
...
# 27 "mcc_generated_files/spi_types.h"
typedef enum {
MASTER0_CONFIG,
SPI_DEFAULT
} spi_modes;

# 35 "mcc_generated_files/spi_driver.h"
...
data[2] = spi_exchangeByte(0);
data[1] = spi_exchangeByte(0);
data[0] = spi_exchangeByte(0);
...
value |= PORTBbits.RB4 << i;

else

value |= PORTBbits.RB4 << (7 - i);
